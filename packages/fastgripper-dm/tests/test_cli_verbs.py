...
def _boom(*a, **k):
    from fastgripper_dm.port import PortError
    raise PortError("simulated failure while printing status")


def test_cmd_status_shuts_the_bus_down_when_the_body_raises(tmp_path, monkeypatch):
    """The teardown is in a finally: a failure while printing (or in disable())
    must still close the port and shut the bus down."""
    import json
    from argparse import Namespace
    ...
    from fastgripper_dm.facade import FastGripper
    from fastgripper_dm.tools import _cli
    from test_facade import ENTRY, FakePort

    cal = tmp_path / "gripper_cal.json"
    cal.write_text(json.dumps({"format": 2, "grippers": {"default": dict(ENTRY)}}))
    port = FakePort(wrapped=1.0, fail_disable=True)   # and disable() raises too

    def fake_standalone(cls, **kw):
        return FastGripper(port=port, cal_path=str(cal), gripper="default", home=kw.get("home"))

    monkeypatch.setattr(FastGripper, "standalone", classmethod(fake_standalone))
    monkeypatch.setattr(cli_mod, "print", _boom, raising=False)
    codes = []
    monkeypatch.setattr(_cli.os, "_exit", codes.append)

    cli_mod.cmd_status(Namespace(gripper=None, cal=str(cal)), [])

    assert codes == [1]                               # PortError surfaced, exit 1
    assert port.closed
    assert port.motor.bus.shutdown.call_count == 1
